...
* Đọc thêm **Plan** (`docs/plans/plan_T-xx.md`) và mục **Plan vs Actual** trong Summary; xem **diff của nhánh `task/T-xx-...`** để đối chiếu với Code Inventory.
...
*   **An toàn:** không commit secret (mật khẩu, key, `.env`), không có thay đổi phá hủy ngoài phạm vi; commit đúng format `T-xx: [FR-xx] ...`.

## 3. Verdict, Cập nhật trạng thái & Vòng lặp (doc-00 Mục 10.6)
*   **Verdict phân cực:** mỗi task nhận **Approved** hoặc **Changes Requested**.
*   **Quy tắc ra verdict theo severity:** có **ít nhất một finding Blocker hoặc Major** → **Changes Requested**. Chỉ có **Minor** (hoặc không có finding) → **Approved**, kèm danh sách Minor trong Review Report để coder xử lý sau (không chặn task, không đưa task về `In-Progress`).
...
*   **Changes Requested** → chỉ cập nhật `Review-Fail` (KHÔNG tự đổi sang `In-Progress`), trả về `coder_agent` kèm Review Feedback — `coder_agent` sẽ chuyển task về `In-Progress` khi bắt đầu sửa.
